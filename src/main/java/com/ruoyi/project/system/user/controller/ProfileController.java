...
//    private PasswordService passwordService;
    ...
    public AjaxResult updateAvatar(@RequestParam("avatarfile") MultipartFile file, HttpServletRequest request) {
        ...
        try {
            if (!file.isEmpty()) {
                //判断SOP文件夹是否存在
                String sopPath = RuoYiConfig.getProfile() + "sop" + currentUser.getCompanyId();
                ...
                currentUser.setLoginTag(UserConstants.LOGIN_TAG_ADD);
                currentUser.setAvatar(imgUrl + "sop" + currentUser.getCompanyId() + "/" + avatar);
                String imgurl = imgUrl + "sop" + currentUser.getCompanyId() + "/" + avatar;
                if (userService.updateUserInfo(currentUser, request) > 0) {
                    setSysUser(userService.selectUserById(currentUser.getUserId()));
